...
namespace landrope.hosts
{
	...
	public class BundleHost : IBundleHost
	{
		...
		public IAssignmentHost GetAssignmentHost() => AssignHost;
		public GraphHostConsumer GetGraphHost() => graph;

		public BundleHost(IServiceProvider services, IConfiguration configuration, IAssignmentHost ashost)
		{
			...
			this.graph = services.GetService<GraphHostConsumer>();
			this.AssignHost = ashost;
				...
				.Select(g => (mbkey: g.Key.key,
								doclist: g.GroupBy(d => (d.keyDocType, d.chainkey))
									.Select(g1 => (DTkey: g1.Key.keyDocType, chain: g1.GroupBy(d1 => d1.chainkey)
										.Select(g2 => (chainkey: g2.Key, props: g2.FirstOrDefault().props, parts: g2.FirstOrDefault().m.current)).ToList())).ToList()
										)).ToList();
			/*									)).ToList();*/
			mbx.ForEach(m =>
			{
		...
		public (ITaskBundle bundle, string reason) MakeTaskBundle(string token, string asgdtlkey, bool save = true)
		{
			...
			var asgn = this.AssignHost.OpenedAssignments().Cast<Assignment>()
							.FirstOrDefault(a => a.details.Any(d => d.key == asgdtlkey));
			if (asgn == null)
		...
		public (ITaskBundle bundle, string reason) DoMakeTaskBundle(Assignment asgn, AssignmentDtl asgdtl, MainBundle bundle, user user, bool save = true, string token=null)
		{
			...
			lock (bundle) //prevents another creation of taskbundle for the same persil/main bundle
			{
				...
				((AssignmentHost)AssignHost).Update(asgn, false);
		...
		{
			var user = contextplus.FindUser(token);
			var assign = (Assignment)AssignHost.GetAssignment(asgnkey);
			if (assign == null)
		...
		/*public */IEnumerable<PersilNx> NextReadies(bool loose = false)
		{
			var persils = CleanPersil;
			if (persils.Length == 0)
				return new PersilNx[0];

			var assigns = AssignHost.OpenedAssignments().Cast<Assignment>().ToArray();
					...
					IEnumerable<(string key, string disc, DocProcessStep next)> nexts)
		{
			...
			var nextsteps = nextstepsrn.Where(x => x.r.disc != null)
							.Select(x => (x.n.key, x.n.disc, step: x.n.next, x.r.keyDocType, exis: x.r.exis));
					...
					IEnumerable<(PersilPositionView ppv, DocProcessStep next)> nexts)
		{
			...
			var nextsteps = nextstepsrn.Where(x => x.r.disc != null && looses.Contains(x.r.step))
							.Select(x => (x.n.ppv, x.n.disc, step: x.n.next, x.r.keyDocType, exis: x.r.exis));
		...
		/*public */IEnumerable<Persil> NextReadies(string akey, bool loose = false, params string[] exceptkeys)
		{
			if (!(AssignHost.GetAssignment(akey) is Assignment assign))
				return new Persil[0];
			...
			var exckeys = assign.details.Select(d => d.keyPersil).Union(exceptkeys);
			persils = persils.Where(p => p.basic.current.keyDesa == assign.keyDesa && (
						p.basic.current.keyPTSK == (assign.keyPTSK ?? assign.key) ||
						p.basic.current.keyPenampung == (assign.keyPenampung ?? assign.key)) &&
						!exckeys.Contains(p.key)).ToArray();

			var nexts = NextSteps(persils, assign).Where(x => x.next == assign.step.Value);
			IEnumerable<(string key, string disc, DocProcessStep step)> phase1;
			if (loose)
				phase1 = nexts;
			else
			{
				var sends = StepDocType.List.Where(s => s.invalid != true && s._step == assign.step.Value && s.send.Any())
									.Join(Assignment.StepOrders.SelectMany(so => so.Value.Select(v => (so.Key, v.step))),
										s => (s.disc, s._step), so => (so.Key, so.step),
										(s, so) => s)
		...
		/*public */IEnumerable<Persil> NextReadiesDtl(string keyDesa, string keyComp, string disc, DocProcessStep step)
		{
			var persils = CleanPersil;
			if (persils.Length == 0)
				return new Persil[0];
		...
		public bool MainUpdate(string key, bool dbSave = true)
		{
			var bundle = MainBundles.FirstOrDefault(b => b.key == key);
			if (bundle == null)
				return false;
		...
		public bool TaskUpdate(string key, bool dbSave = true)
		{
		...
		public Persil[] CleanPersil => contextplus.persils.Query(p => (p.en_state == null || p.en_state == StatusBidang.bebas) && p.invalid != true &&
		...
		*/
		/*public */PersilPositionView[] LastPositionDiscrete(Persil[] persils = null, IAssignment[] iassigns = null)
		{
			...
			var pactive = ((AssignmentHost)AssignHost).AssignedPersils().ToArray();
			...
			unifieds = unifieds2.Join(PBTs, x => x.noPBT, p => p.noPBT, (x, p) => (x.d.key, x.d.IdBidang, x.d.keyProject, x.d.keyDesa, x.d.keyPTSK, x.d.keyPenampung,
												 ...
												 luasProp: (double?)x.d.luasDibayar * p.luas / x.luas, noPBT: p.noPBT, x.d.noHGB)).ToArray();
		...
		public List<PersilPositionView> SavedPersilPosition(bool inclActive=false) =>
			contextplus.GetCollections(new PersilPositionView(), PersilPositionCollname, inclActive?"{}" : "{ongoing:false}", "{_id:0}").ToList();
